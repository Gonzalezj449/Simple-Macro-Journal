...
class ChatWithDoc(tk.Frame):
    ...
    def create_widgets(self):
        # Label for the title
        title_label = tk.Label(self, text="User Dashboard: Chatting With Doctor...", font=("Arial", 20))
        ...
        self.doctor_chat_display.config(state=tk.DISABLED)  # Make the text area read-only
        ...
        send_button = tk.Button(self, text="Send", command=self.send_doctor_message)
    ...
    def display_messages(self):
        for message in self.messages:
            ...
            msg = message["message"]
            self.doctor_chat_display.config(state=tk.NORMAL)  # Enable editing temporarily
            self.doctor_chat_display.insert(tk.END, f"{sender}: {msg}\n")
            self.doctor_chat_display.config(state=tk.DISABLED)  # Make the text area read-only again
    ...
    def go_back(self):
        ...
        self.save_messages()
        self.parent.show_page("DataEntryPage")
